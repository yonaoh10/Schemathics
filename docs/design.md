...
## 2. Dilemma 1 — "do not change the logic" versus a 54 ms feature pipeline

**What I found.** With the models already warm, one request through the research
pipeline costs 54 ms p50, and almost none of it is arithmetic. It is pandas
...
1. Ship 54 ms and call it done. Defensible on the letter of the brief, indefensible on
...
**Result: 54 ms to 1.8 ms**, with the original one environment variable away.
...
**Measured here** (4 CPU, 15 GB, context 1000x25, one request = 15 brand rows):

| backend | start-up | per request | max diff vs the library default |
|---|---|---|---|
| `tabpfn_local`, `low_memory` | 0.2 s | 37.3 s | $4.03 |
| `tabpfn_local`, `fit_preprocessors` | 0.4 s | 8.69 s | reference |
| `tabpfn_local`, `fit_with_cache` | 14.9 s | **0.26 s** | **$0.000045** |
| `tabpfn_client`, fit reused | weekly | 1 network round trip | exact |
| `surrogate` (CatBoost student) | 5 MB load | **~0.5 ms** | measured and logged |
...
54 ms to 12 ms, entirely by hoisting start-up work. Good, and not enough for a page load.
